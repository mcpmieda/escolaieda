...
A nova guia `Boletim` foi reconstruída do zero contra `C:\Users\Eugui\Desktop\aqui.png`: mantém o shell comum, oferece turma, aluno, trimestres, situações, modo colorido/P&B, impressão/PDF, zoom e tela cheia. A turma selecionada gera todos os boletins fictícios, organizados em folhas A4 verticais com quatro boletins horizontais empilhados por página. O documento usa retrato inteiramente fictício, 12 colunas disciplinares de largura idêntica e a mesma cor azul para notas regulares e nota final. Após validação na página real, o conteúdo interno passou a ser redimensionado por `ResizeObserver`; não remover `--bulletin-content-scale`, pois ele impede rodapé, tabela, círculos e textos de se sobreporem em janelas menores que a referência de 1672 px. A prévia renderiza somente uma folha por vez e navega por controles próprios; Imprimir remonta as folhas para `window.print()` e Baixar PDF gera/download o arquivo diretamente. Não voltar a manter as nove páginas simultaneamente no DOM durante a leitura comum.
