...
- validar com o responsável a versão finalizada de `/notas/#estatisticas` contra o `anexo 7.png` e o refinamento `anexo 12.png`, incluindo `TODAS AS TURMAS`, `VISÃO GERAL`, ranking top 3/top 10 com expansão interna estável e mesma altura de itens, linguagem `NOTAS AZUIS`/`NOTAS VERMELHAS`/`MÉDIA DA TURMA`, busca global com painel próprio, relatório contextual por disciplina, brilho suavizado do donut/legendas, relevo 3D sutil do donut, escala dinâmica do gráfico, barras sem invadir cabeçalho, ícones nos seletores, temas claro/mono e impressão própria, antes de ligar dados reais;
- validar com o responsável a remodelagem `aqui4.txt`/`aqui5.txt`/`aqui6.txt` e as revisões de 09/07/2026 e 10/07/2026 de `/notas/#notas`, incluindo seletores/impressão no padrão de `Estatísticas`, seletores Turma/Período com mesmas dimensões e menus opacos, 35 alunos fictícios por turma, filtro compacto em duas colunas no desktop e opaco/legível, ausência de resumo/dashboard na ficha, ausência da duplicidade `Ficha de notas`, ausência da regra demonstrativa na superfície principal, tabela compacta com pílulas arredondadas, nomes completos, coluna `Status` como segunda coluna em linha única, hover horizontal/vertical suavizado, modal de aluno opaco e legível, rolagem sem atraso causado por animação de linhas/chips, temas claro/mono aplicados à ficha/tabela/filtro, rolagem interna no mobile, insights laterais mais estreitos no padrão do `anexo 10.png` sem nota numérica lateral e resultados fictícios completos, antes de considerar a aba visualmente fechada;
