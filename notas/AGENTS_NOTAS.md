...
- validar com o responsável a remodelagem `aqui4.txt`/`aqui5.txt`/`aqui6.txt` e a revisão dos prints de 09/07/2026 de `/notas/#notas`, incluindo seletores/impressão no padrão de `Estatísticas`, 35 alunos fictícios por turma, filtro compacto na linha da tabela com popover opaco/legível, ausência de resumo/dashboard na ficha, ausência da duplicidade `Ficha de notas`, ausência da regra demonstrativa na superfície principal, tabela compacta com pílulas arredondadas, nomes completos, coluna `Status` como segunda coluna em linha única, hover horizontal/vertical suavizado, modal de aluno opaco e legível, rolagem interna no mobile, insights laterais mais estreitos no padrão do `anexo 10.png` sem nota numérica lateral e resultados fictícios completos, antes de considerar a aba visualmente fechada;
