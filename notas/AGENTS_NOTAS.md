...
Decisão adicional em 10/07/2026 após correção de sobreposição na tabela: qualquer preview, tooltip ou cartão flutuante dentro da tabela de `Notas` deve ficar escondido por padrão (`display: none` ou equivalente) e ser aberto somente por hover/foco explicitamente escopado. Não deixar elementos auxiliares renderizados dentro das células, porque isso cobre nomes, pílulas e notas durante a leitura normal da ficha.
...
- Responsável informou que a aba `Notas` ficou estranha e que apareciam elementos por cima do nome dos alunos na tabela.
- Diagnóstico: na integração anterior, a regra `body[data-view="notas"] .studentPeek` sobrescreveu o `display: none` base da prévia do aluno com `display: grid`. Como cada célula de nome contém um `<aside class="studentPeek">`, todas as prévias/fotos ficavam renderizadas por padrão dentro da tabela e se sobrepunham aos nomes.
- Ajuste feito em `notas/css/componentes.css`:
  - `body[data-view="notas"] .studentPeek` voltou a ficar `display: none` por padrão;
  - a prévia passou a abrir somente pelos seletores escopados `body[data-view="notas"] .notesNameCell:hover .studentPeek` e `body[data-view="notas"] .studentNameButton:focus + .studentPeek`;
  - a prévia foi reposicionada lateralmente (`left: calc(100% + 10px)`, `top: 50%`, `transform: translateY(-50%)`) para não nascer em cima da linha de nomes;
  - `pointer-events: none` evita que a própria prévia capture o mouse e mantenha estados visuais estranhos.
- Ajuste feito em `scripts/testes-notas.mjs`:
  - adicionadas travas para exigir que a prévia do aluno fique escondida por padrão e apareça somente no hover/foco.
- Validações executadas:
  - `node scripts/testes-notas.mjs` passou;
  ...
  - captura local via Playwright CLI em `diagnosticos/notas-corrige-preview-aluno.png` confirmou a tabela sem elementos sobrepostos aos nomes.
...
- Próxima etapa correta: responsável validar `/notas/#notas` publicada; se ainda houver sobreposição, pedir print porque a causa CSS identificada nesta etapa já foi corrigida.
