...
- validar com o responsável a versão finalizada de `/notas/#estatisticas` contra o `anexo 7.png`, incluindo `TODAS AS TURMAS`, `VISÃO GERAL`, ranking top 3/top 10 com expansão interna estável, linguagem `NOTAS AZUIS`/`NOTAS VERMELHAS`/`MÉDIA DA TURMA`, busca global com painel próprio, relatório contextual por disciplina, brilho suavizado do donut/legendas, escala dinâmica do gráfico, ícones nos seletores, temas claro/mono e impressão própria, antes de ligar dados reais;
- validar com o responsável a remodelagem `aqui4.txt` de `/notas/#notas`, incluindo seletores/impressão no padrão de `Estatísticas`, filtro compacto dentro da ficha, tabela com pílulas arredondadas, nomes completos, hover horizontal/vertical, rolagem interna no mobile, insights laterais sem recolhimento e resultados fictícios completos, antes de considerar a aba visualmente fechada;
...
- **Flash visual de painel de outra aba ao carregar**: renderizar todas as views logo após abrir a SPA pode produzir percepção de conteúdo estranho durante o primeiro ciclo de layout. Solução aplicada em 09/07/2026: `abrirView()` renderiza somente a view ativa; as demais views renderizam quando forem abertas ou quando seus controles mudarem.
