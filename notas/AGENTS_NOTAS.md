...
Decisão adicional em 08/07/2026 para a aba `Notas`: ao levar a linguagem de `Estatísticas` para telas tabulares, preservar densidade e leitura acima de efeitos decorativos. Selects nativos devem virar seletores customizados quando estiverem no fluxo principal; cards de informação devem trazer ícones; filtros frequentes devem parecer chips acionáveis; tabelas podem manter rolagem interna no mobile, mas não podem alargar a página inteira. A renderização de uma view não deve sobrescrever título/subtítulo do cabeçalho superior de outra view.
...
- validar com o responsável a primeira remodelagem de `/notas/#notas`, incluindo seletores customizados, cards com ícones, chips de situação, tabela com rolagem interna no mobile, insights e lista de recuperação, antes de considerar a aba visualmente fechada;
