...
- validar com o responsável a remodelagem `aqui4.txt`/`aqui5.txt` de `/notas/#notas`, incluindo seletores/impressão no padrão de `Estatísticas`, 35 alunos fictícios por turma, filtro compacto na linha da tabela, ausência de resumo/dashboard na ficha, tabela com pílulas arredondadas, nomes completos, coluna `Status` como segunda coluna, hover horizontal/vertical, modal de aluno opaco, rolagem interna no mobile, insights laterais no padrão do `anexo 10.png` e resultados fictícios completos, antes de considerar a aba visualmente fechada;
