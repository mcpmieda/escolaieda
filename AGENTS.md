...
> **Versão operacional atualizada em 28/05/2026 — preparada para Codex executar diagnósticos e correções diretamente.**  
...
### 0.2. Orientação explícita para o Codex

Este AGENTS orienta o Codex no PowerShell a executar diagnósticos e correções diretamente no repositório **quando o usuário pedir uma tarefa específica**.

O Codex **não deve** iniciar fases por conta própria apenas porque leu o AGENTS.

Para cada tarefa, o Codex deve:

```text
1. ler este AGENTS.md;
2. confirmar o estado real do projeto com Git;
3. executar somente a fase/tarefa pedida pelo usuário;
4. começar por diagnóstico somente leitura em fases novas ou sensíveis, salvo ordem explícita para aplicar correção;
5. alterar somente o necessário;
6. não avançar para a próxima fase sem nova autorização.
```

### 0.3. Fluxo obrigatório
...
### 0.5. Fonte da verdade sobre o estado atual

O AGENTS traz o contexto do projeto, mas o estado real deve ser confirmado no Git antes de qualquer ação:
...
Não confiar apenas no histórico escrito neste documento para decidir se uma tag, commit ou alteração já existe localmente.
...
## 2. Preferências do usuário e fluxos de trabalho
...
### 2.1. Fluxo com ChatGPT no navegador

Quando a alteração for preparada pelo ChatGPT no navegador, o fluxo pode ser:
...
Esse fluxo existe para facilitar quando o ChatGPT não está operando diretamente dentro do repositório local.

### 2.2. Fluxo com Codex no PowerShell

Quando a tarefa for feita pelo Codex no PowerShell, o Codex pode diagnosticar e alterar arquivos diretamente no repositório.

Regra para o Codex:

```text
Codex pode fazer diagnósticos e correções diretamente no projeto quando solicitado.
Codex deve seguir rigorosamente este AGENTS.md.
Codex deve confirmar o estado do Git antes de agir.
Codex deve gerar backup antes de alterar arquivo sensível.
Codex deve gerar relatório em diagnosticos.
Codex deve validar JS e rodar git diff --check.
Codex deve aguardar teste/confirmação do usuário antes de commit/push/tag, salvo autorização explícita.
```

Para fases novas ou sensíveis:

```text
Começar por diagnóstico somente leitura.
Não alterar arquivos ainda.
Gerar relatório em diagnosticos.
Aguardar confirmação do usuário para aplicar.
```

Para correções já autorizadas explicitamente pelo usuário:

```text
Aplicar somente a correção pedida.
Não aproveitar para fazer outras fases.
Não fazer alterações oportunistas.
```
...
### 12.2. Executar script enviado pelo ChatGPT, quando esse fluxo for usado
...
## 12.6. Prompt seguro para iniciar no Codex

Quando iniciar o Codex, o usuário pode usar:
...
```text
Codex não deve executar várias fases na mesma resposta.
Codex não deve fazer commit/push/tag sem autorização.
Codex não deve usar git add . como padrão.
```
...
7. **Blocos de exemplos excessivamente longos**
...
## 18. Primeira ação recomendada ao retomar

Se o usuário disser “vamos continuar”, a primeira ação deve ser confirmar o estado real do projeto:
